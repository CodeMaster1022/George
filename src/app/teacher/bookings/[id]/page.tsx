                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap items-center gap-3">
                          {booking.session.meetingLink ? (
                            <>
                              <div className="flex-1 min-w-0 text-gray-600 text-sm font-mono bg-gray-50 px-3 py-2 rounded border truncate">
                                {booking.session.meetingLink}
                              </div>
                              {safeJoinHref(booking.session.meetingLink) && (
                                <div className="flex flex-col gap-2">
                                  <JoinMeetingButton
                                    ...
                                    t={t}
                                    className="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-lg bg-[#0058C9] hover:bg-[#0046A3] text-white text-sm font-medium transition-colors"
                                  >
                                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          ...
                          ) : getMeetingTimeStatus(booking.session.startAt, booking.session.endAt) === "active" ? (
                            <button
                              type="button"
                              disabled={generatingLink}
                              onClick={async () => {
                                setGeneratingLink(true);
                                const r = await apiJson<{ meetingLink: string }>(`/teacher/bookings/${encodeURIComponent(booking.id)}/generate-meeting-link`, { method: "POST", auth: true });
                                setGeneratingLink(false);
                                if (r.ok) await loadBooking();
                              }}
                              className="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-lg bg-amber-600 hover:bg-amber-700 text-white text-sm font-medium transition-colors disabled:opacity-60"
                            >
                              {generatingLink ? t("generatingMeetingLink") : t("generateMeetingLink")}
                            </button>
                          ) : (
                            <span className="text-gray-500 text-sm">{t("generateMeetingLinkAtTime")}</span>
                          )}
                        </div>
                      </div>
                    )}
